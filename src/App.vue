...
<template>
  <div :class="nightMode ? darkModeSet('bg') : ''" class="h-full">
    ...
    <!-- How to play pop up-->
    <div
      class="fixed top-0 left-0 right-0 z-50 w-full bg-black/50 flex items-center justify-center h-screen"
      v-show="showHowtToPlay"
    >
      <div
        class="bg-slate-200/90 rounded-lg text-black w-3/4 relative shadow-2xl h-5/6"
      >
        <button
          class="absolute right-0 p-6 hover:underline"
          @click="showHowtToPlay = !showHowtToPlay"
        >
          close
        </button>
    ...
    <div class="flex justify-center w-full" id="catagory">
      <div>
        <h1
          class="text-center text-3xl font-bold mt-14"
          :class="nightMode ? darkModeSet('font') : 'text-black'"
        >
          Choose a catagory
        </h1>
        <div class="text-center mt-10 grid md:grid-cols-3 gap-4 grid-cols-2">
          <a
            href="#level"
            v-for="(cg, key) in catagories"
            :key="key"
            @click="
              category = catagories[key].reduce((a, b) =>
                a.length > b.length ? a : b
              );
              selectCTG = key;
            "
            :class="
              catagories[key].includes(category)
                ? 'bg-slate-700 text-white'
                : 'bg-slate-300' && nightMode
                ? darkModeSet('block')
                : 'bg-slate-300 text-black hover:bg-black hover:text-white'
            "
    ...
    <div class="flex justify-center w-full cursor-pointer" id="level">
      <div>
        ...
        <div
          class="mt-10 md:grid md:grid-cols-3 gap-4 flex flex-col items-center"
        >
          <a
            href="#letgo"
            v-for="(level, key) in difficulties"
            :key="key"
            @click="difficulty = level.name.toLowerCase()"
            :class="
              level.name.toLowerCase() === difficulty
                ? level.selectBg
                : level.bg
            "
        ...
        <a href="#question">
          <div class="flex justify-end" id="letgo">
            ...
            <button
              class="rounded-lg p-1.5 mt-4 w-28 mb-14 shadow-lg"
              :class="nightMode ? darkModeSet('btn') : 'bg-black text-white'"
              @click="checkOption()"
            >
              Let's play
            </button>
    ...
    <!-- warn pop up-->
    <div
      class="fixed top-0 left-0 right-0 z-50 p-4 grid place-content-center bg-black/50 h-screen"
      v-show="warnMode"
    >
      <div class="relative w-full h-full max-w-md md:h-auto">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
          ...
          <div class="p-6 text-center">
            ...
            >
              Please select category or level
            </h3>
          </div>
        </div>
      </div>
    ...
    <div v-show="showQuestion" class="h-screen mt-20">
      ...
      <div class="md:flex justify-center hidden">
        <div
          id="navbar"
          class="rounded-2xl p-4 w-5/6 grid grid-cols-3 shadow-lg mt-9"
          :class="nightMode ? darkModeSet('block') : 'bg-slate-200'"
        >
          <div
            class="cursor-pointer flex justify-start hover:bg-slate-500/20 w-fit h-7 rounded-lg items-center"
          >
            <a href="#head">
              <p
                class="p-2"
          ...
          <div class="flex justify-center cursor-pointer">
            <p
              class="text-black text-center bg-green-300 rounded-lg px-2 grid content-center"
            >
              score {{ score }}
            </p>
          </div>
          <div class="flex justify-end mr-2 cursor-pointer">
            <p :class="nightMode ? darkModeSet('font') : 'text-black'">
              Inw Trivia
            </p>
          </div>
        </div>
      ...
      <!-- question -->
      <div
        class="flex flex-col items-center md:mt-20 mt-10 justify-center mb-10"
        id="question"
      >
        <div class="text-center text-black space-y-6 w-3/5 break-words mt-10">
          <div
            class="p-6 rounded-2xl tex-center grid content-center shadow-lg"
            :class="nightMode ? darkModeSet('block') : 'bg-slate-200'"
          >
            <p class="text-sm">Question {{ currentQuestion }}/10</p>
            <p class="md:hidden">
              Score <span class="text-teal-400">{{ score }}</span>
            </p>
            <h1 class="text-2xl md:text-3xl break-words">
              {{ question[currentQuestion - 1]?.question }}
            </h1>
          </div>
          <div
            class="grid grid-cols-1 md:grid-cols-2 rounded-2xl p-2 shadow-lg"
            :class="nightMode ? darkModeSet('block') : 'bg-slate-200'"
          >
            <div
              ...
              @click="
                choiceOnClick(
                  choice,
                  question[currentQuestion - 1]?.correctAnswer
                );
                showResult();
              "
              :class="
                showAnswer
                  ? question[currentQuestion - 1]?.correctAnswer === choice
                    ? 'bg-green-400'
                    : 'bg-red-500'
                  : 'bg-teal-500'
              "
    ...
    <!-- result -->
    <div
      class="h-screen bg-black/50 fixed top-0 left-0 right-0 z-50 w-full"
      v-show="showResultPopup"
    >
      <div class="fixed top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2">
        <div
          class="relative w-full h-full max-w-2xl md:h-auto grid content-center"
        >
          <!-- content -->
          <div
            class="relative bg-white rounded-3xl shadow text-black grid content-center p-6"
          >
            <div
              class="flex flex-col items-start justify-between p-4 space-y-2 rounded-t dark:border-gray-600"
            >
            ...
            <div class="flex justify-center">
              <div
                class="p-6 space-y-5 bg-slate-200 w-36 h-36 rounded-full text-center"
              >
            ...
            <div
              class="flex items-center p-6 space-x-2 justify-center rounded-b dark:border-gray-600"
            >
              <a href="#head">
                <button
                  @click="setNewQuiz()"
