...
const themeDark = ref({ 'background-color': 'black',
                        'color': '#fff' ,
                        'border-color': '#fff'})
                        
const themeWhite = ref({ 'background-color': '#1f1f1f', 'color': '#fff' })
const blockDark = ref({ 'background-color': '#3f3f46',
                        'color': '#fff' ,
                        })

const fontDark = ref({ 'color': '#fff' })


</script>

<template>
  <div :style="nightMode === true ? themeDark : ''">
    ...
    <div class="w-full h-screen flex items-center justify-center" id="head">
      <div class="md:grid md:grid-cols-2 content-center w-full">
        <div class="flex flex-col justify-center md:ml-24 text-center md:text-left">
          <h1  :style="nightMode === true ? fontDark : ''"
    ...
    <div class="flex justify-center w-full" id="catagory">
      <div>
        <h1 class="text-center text-3xl text-black font-bold mt-14" :style="nightMode === true ? fontDark : ''">Choose a catagory</h1>
        <div class=" text-center mt-10 grid md:grid-cols-3 gap-4  grid-cols-2">
          ...
          :style="nightMode === true ? blockDark : ''"  
    ...
    <div class="flex justify-center cursor-pointer" id="level">
      <div>
        <h1 class="text-center text-3xl text-black font-bold mt-10">Which level do you want</h1>
        ...
        <a href="#question">
          <div class="flex justify-end" id="letgo">
            ...
            <button class="bg-black text-white rounded-lg p-1.5 mt-4 w-28 mb-14 shadow-lg"
    ...
    <div v-show="showQuestion" class="h-screen">
      ...
      <div class="md:flex justify-center hidden">
        <div id="navbar" class="bg-slate-200 rounded-2xl p-4 w-5/6 grid grid-cols-3 shadow-lg mt-9" v-show="closeQuiz">
          <p class="text-black flex justify-start">back</p>
          <div class="flex justify-center">
            <p class="text-black text-center bg-green-300 rounded-lg px-2 grid content-center">score {{
              score
            }}</p>
          </div>
          <label class="swap swap-rotate neutral justify-end mr-6">
      ...
      <div class=" flex flex-col items-center md:mt-40 mt-10 justify-center  mb-20 " v-show="closeQuiz" id="question">
        <div class="text-center text-black space-y-6 w-3/5 break-words">
          <div class="bg-slate-200 p-6 rounded-2xl tex-center grid content-center shadow-lg">
            <p class="text-sm">Question {{ questionNum }}/10 &nbsp; <span class=" md:hidden">
                score {{ score }}</span></p>
            <h1 class="text-2xl  md:text-3xl break-words">{{ q }}</h1>
          </div>
          <div class="grid grid-cols-1 md:grid-cols-2 bg-slate-200 rounded-2xl p-2 shadow-lg">
            <div v-for="(c, index) in shuffle(ch)" :key="index" :id="c"
              ...
              class="bg-teal-500 p-2 pl-4 pr-4 rounded-lg m-2 hover:bg-black hover:text-white w-84 shadow-lg">
              <button>
                {{ c }}
              </button>
            </div>
      ...
      <div v-show="closeQuiz" class="flex justify-center mt-4 mb-9">
        <p class="text-black">All question are from <a href="https://the-trivia-api.com/">"The Trivia API"</a></p>
      </div>
