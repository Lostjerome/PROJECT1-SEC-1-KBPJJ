...
const score = ref(0)
const checkFinishTest = ref(false)
const countUserClick = ref(0)
...
function getChoice(choiceTarget){
  return shuffle([choiceTarget.correctAnswer,...choiceTarget.incorrectAnswers])
}
...
<template>
 <div class="min-h-screen min-w-full flex flex-col items-center bg-gray-400">
   ...
   <!-- nav-Bar -->
      ...
      <div class=" flex flex-col bg-white w-full h-auto mt-10 items-center justify-center rounded-3xl py-7 px-3">
       ...
       <!-- Choice -->
        <div class="grid grid-cols-1 gap-3 bg-slate-300 mt-5 py-5 px-10 rounded-3xl w-full lg:grid-cols-2">
            <div v-for =" (getC,index) in getChoice(questionArr[currentTG]) " :key="index" id="getC"
            @click=" userClick = getC; 
                     userClick === questionArr[currentTG].correctAnswer ? score++ :score +=0; 
                     currentTG == questionArr.length-1 ? currentTG+=0 : currentTG++; 
                     countUserClick++; 
                     countUserClick > questionArr.length-1 ? checkFinishTest = true :checkFinishTest =false;
                 ">
        ...
        <div class="flex mt-7 justify-center">
          <button class="  h-9 w-9 bg-white rounded-full mr-2 hover:bg-slate-400"
          @click=" score = 0; currentTG = 0; checkFinishTest=false; countUserClick=0; getQuestion();">RL</button>
        </div>
    ...
    <!-- result-score -->
    <div class="bg-fixed absolute min-w-full min-h-screen flex flex-col items-center" v-show="checkFinishTest">
        <div class=" flex flex-col w-3/6 h-full bg-white text-center mt-40 rounded-3xl p-2 items-center">
          ...
          <!-- button-action -->
           <div class=" flex flex-col gap-4 font-bold mt-5 py-4 w-3/4 text-center"> 
               <!--button-play-again -->
              <button class=" bg-blue-500 text-white rounded-full w-full py-2 hover:bg-blue-600 "
              @click=" score = 0; 
                       currentTG = 0; 
               ...
               <!--button-home -->
              <button class=" bg-gray-100 rounded-full p-2 py-2 hover:bg-gray-200"
              @click="reloadPage();"
                  >Home
              </button>
          </div>
